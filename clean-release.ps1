...
function Remove-DirectorySafe {
    ...
    if (Test-Path $Path) {
        try {
            ...
            Write-Host "  [OK] Cleaned $Description" -ForegroundColor Green
        }
        catch {
        ...
        if ($Verbose) {
            Write-Host "  [SKIP] $Description not found (already clean)" -ForegroundColor Gray
        }
    }
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] dotnet clean completed successfully" -ForegroundColor Green
    }
    else {
    ...
    Write-Host "[ERROR] Error running dotnet clean: $($_.Exception.Message)" -ForegroundColor Red
}
...
if (-not $KeepRelease) {
    ...
    Write-Host "  [SKIP] Keeping release folder (use without -KeepRelease to remove)" -ForegroundColor Gray
}
    ...
    if ($files) {
        foreach ($file in $files) {
            try {
                ...
                Write-Host "  [ERROR] Failed to remove temp file $($file.Name): $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        Write-Host "  [OK] Cleaned temporary files ($($files.Count) files)" -ForegroundColor Green
    }
    else {
        if ($Verbose) {
            Write-Host "  [SKIP] No temporary files found ($pattern)" -ForegroundColor Gray
        }
    }
}
...
try {
    ...
    Write-Host "Workspace size: $sizeInMB MB" -ForegroundColor Cyan
}
catch {
...
Write-Host "  clean-release.ps1                 - Clean everything" -ForegroundColor White
Write-Host "  clean-release.ps1 -KeepRelease    - Clean but keep release folder" -ForegroundColor White
Write-Host "  clean-release.ps1 -Verbose        - Show detailed output" -ForegroundColor White
Write-Host ""
